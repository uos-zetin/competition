...
import { ConnectionStatusBadge, counterService } from "@/entities/counter";
...
import { CurrentRunnerPanel } from "./current-runner-panel";
...
export function ManualCounterPage() {
  ...
  const isConnected = counterService.use.isConnected(counterId ?? "");
  ...
  useEffect(() => {
    ...
    void counterService.connection.connect(counterId).catch((error: unknown) => errorHandlingService.handle(error, "계수기 연결에 실패했습니다"));
    return () => {
      void counterService.connection.disconnect(counterId).catch((error: unknown) => errorHandlingService.handle(error, "계수기 연결 해제에 실패했습니다"));
    };
  }, [counterId, isAuthorized]);

  useEffect(() => {
    if (!isAuthorized || !counter?.divisionId) return;
    void progressService.connection.connect(counter.divisionId).catch((error: unknown) => errorHandlingService.handle(error, "진행 상황 연결에 실패했습니다"));
    return () => {
      void progressService.connection.disconnect().catch((error: unknown) => errorHandlingService.handle(error, "진행 상황 연결 해제에 실패했습니다"));
    };
  ...
  const resolvedParticipant = pinnedParticipant ?? runner?.participant ?? null;
  ...
  const startStopwatch = () => {
    if (!runner) return;
    setPinnedParticipant(runner.participant);
      ...
      <PageContainer maxWidth="md" padding="md" className="py-6 sm:py-8">
        <div className="mx-auto flex max-w-xl flex-col gap-4">
          <div className="flex items-center justify-between gap-3 px-1 text-sm text-muted-foreground">
            <span className="truncate">계수기: <strong className="font-semibold text-foreground">{counter?.name ?? counterId}</strong></span>
            <ConnectionStatusBadge connected={isConnected} />
          </div>
          <CurrentRunnerPanel participant={resolvedParticipant} divisionId={counter?.divisionId ?? null} divisionName={division?.name ?? null} />
          <StopwatchPanel startedAt={startedAt} stoppedAt={stoppedAt} noRunner={!runner} onStart={startStopwatch} onStop={stopStopwatch} onReset={resetStopwatch} />
          <SubmitPanel recorderName={recorderName} onRecorderNameChange={setRecorderName} canSubmit={canSubmit} hint={hint} onSubmit={() => void submit()} successToken={successToken} successMessage="기록을 전송했습니다." />
