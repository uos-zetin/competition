...
export function createProgressService({ progressRepository, progressChannel }: { progressRepository: ProgressRepository; progressChannel: ProgressChannel }) {
  ...
  return {
    load: { byDivision: async (divisionId: string): Promise<void> => useProgressStore.getState().setProgress(await progressRepository.getProgress(divisionId)) },
    admin: {
      ...
      setCurrentRunner: (divisionId: string, participantId: string) => progressRepository.setCurrentRunner(divisionId, participantId),
    },
    connection: {
